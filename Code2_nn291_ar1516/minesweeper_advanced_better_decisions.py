...
def update_probabilities(game,dim,tuples,knowledge): #update probabilities of remaining spots
    
    probabilities = []
    for i in range(len(tuples)): #go through remaining spots left to choose from
        r = tuples[i][0]
        c = tuples[i][1]
        temp = False
        prob = 1
        ...
        #probability is equal to 1 divided by the clue of the neighbor around it
        for n in neighbors:
            temp = False
            if game.grid[n.r][n.c].uncovered == True:
                if game.grid[n.r][n.c].total_mines != 0:
                    ...
                    temp = True
        if not temp:
            heapq.heappush(probabilities, (prob,(r,c))) #push to a priority queue with calculated probability
        else:
            heapq.heappush(probabilities, (0,(r,c))) #push to a priority queue with priority 0 since the spot is safe
    ...
    while not done: #start while loop
        ...
        if not done:
            if current_x != -1 and current_y != -1: #if game is still running
                ...
                probabilities = update_probabilities(game,dim,tuples,knowledge) #calculate probabilities for the game board
                ...
                if len(tuples) > 0 or len(knowledge) > 0: #if there are spots left on the board
                    if len(knowledge) > 0:
                        ...
                        #instead of randomly picking, use probability calculations to pick next spot
