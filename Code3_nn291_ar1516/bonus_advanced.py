...
def get_to_next(game,current,next_state): #travel between starting and final states
	'''
    while True:      
        neighbors = current.get_neighbors(game.grid,game.dim) #find all neighbors
        distances = []
        
        for n in neighbors: 
            distances.append(find_distance(n,next_state))

        min_dist = min(distances) #check which neighbor is closest to the final state
        current = neighbors[distances.index(min_dist)]

        game.steps = game.steps + 1

        if (current.r == next_state.r and current.c == next_state.c):
            return
    '''
...
def find_range(game,current):
    ...
    #for i in range( len(ranges)):
        #print(ranges[i].r,ranges[i].c)

    #print("Ranges length: ",len(ranges))
        
    return ranges

def highest_state(game, current): #returns closest state with high probability of being the moving target based on observations so far
    ...
    distances_in_five = []
    in_five = []
    distances_not_in_five = []
    not_in_five = []

    for i in range(len(ranges)):
        d = find_distance(current,ranges[i])
        if d <= 5:
            in_five.append(ranges[i])
            distances_in_five.append(d)
        else: 
            not_in_five.append(ranges[i])
            distances_not_in_five.append(d)

    ans = current
    dist = find_distance(current, target_cell)
    if dist <= 5 and len(distances_in_five) > 0:
        ans = ranges[distances_in_five.index(min(distances_in_five))]
    elif dist > 5 and len(distances_not_in_five) > 0:    
        ans = ranges[distances_not_in_five.index(min(distances_not_in_five))]
        ...
        if not done:
            if not target_found:
                ...
                if current.target == True: 
                    ...
                    if dist_real <= dist_temp: #possibly add something to define a range of
