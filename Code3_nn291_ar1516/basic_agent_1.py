...
def get_to_next(game,current,next_state): #travel between current and next state to search, adjust time steps accordingly
...
def highest_state(game, current): #returns state with highest probability of being the target based on observations so far
    max_p = -1
    ans = current
    for r in range(game.dim): 
        for c in range(game.dim): 
            if not (r == current.r and c==current.c):
                if game.grid[r][c].p > max_p:
                    max_p = game.grid[r][c].p
                    ans = game.grid[r][c]
                elif game.grid[r][c].p == max_p:
        ...
        if not done:
            if not target_found:
                ...
                if current.target == True: 
                    ...
                    next_state = highest_state(game, current) #declares next state to be state with the highest probability of being the target
