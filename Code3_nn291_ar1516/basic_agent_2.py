...
def highest_state(game, current): #returns state with highest probability of being FOUND as the target based on observations so far
    max_p = -1
    ans = current
...
def update_info(game, current): #update knowledge
    ...
    if current.target == True: 
        ...
        current.p_found = ((1-current.fn) * current.p) #calculate probability of finding the target 
        ...
        if not done:
            if not target_found:
                ...
                if current.target == True: 
                    ...
                    next_state = highest_state(game, current) #declares next state to be state with the highest probability of being FOUND as the target
